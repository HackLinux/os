...
/*! 割込みコントローラ(MIR有効化) */
extern void intc_enable_irq(INTRPT_TYPE irq);

/*! 割込みコントローラ(MIR無効化) */
extern void intc_disable_irq(INTRPT_TYPE irq);

/*! CPSRの外部割込み(IRQとFIQ)有効化チェック */
extern int is_ext_intr_enable(void);

/*! CPSRの外部割込み(IRQとFIQ)有効化 */
extern void enable_exter_intr(void);

/*! CPSRの外部割込み(IRQとFIQ)無効化 */
extern void disable_exter_intr(void);

/*! CPSRのIRQ割込み有効化チェック */
extern int is_irq_enable(void);
